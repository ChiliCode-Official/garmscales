...
const prepareHeadingFalls=()=>document.querySelectorAll('main h2,main h3').forEach(title=>{if(title.dataset.accelReady||title.closest('.spotify-card'))return;title.dataset.accelReady='true';title.innerHTML=[...title.textContent].map(char=>`<span class="accel-letter">${char===' '?'&nbsp;':char}</span>`).join('')});accelMode?.addEventListener('change',()=>{if(accelMode.checked)prepareHeadingFalls()});document.addEventListener('click',event=>{if(!accelMode?.checked||event.target.closest('.accel-control,.spotify-launch,.spotify-panel'))return;event.stopImmediatePropagation();accelBurst.style.left=`${event.clientX}px`;accelBurst.style.top=`${event.clientY}px`;accelBurst.classList.remove('is-visible');void accelBurst.offsetWidth;accelBurst.classList.add('is-visible');document.querySelectorAll('.accel-letter').forEach(item=>{item.style.setProperty('--x',`${(Math.random()-.5)*80}vw`);item.style.setProperty('--r',`${(Math.random()-.5)*1080}deg`);item.style.setProperty('--delay',`${Math.random()*.45}s`);item.classList.add('is-launched');setTimeout(()=>item.classList.remove('is-launched'),1800)})},true);
